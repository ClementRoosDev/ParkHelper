...
namespace ParkHelper.Apiv2.Controllers
{
    ...
    */
    public class PlanningsController : ODataController
    {
        ...
        {
            return SingleResult.Create(db.Plannings.Where(planning => planning.id == key));
        }
        ...
        {
            return SingleResult.Create(db.Plannings.Where(m => m.id == key).Select(m => m.EtatLieu));
        }
        ...
        {
            return SingleResult.Create(db.Plannings.Where(m => m.id == key).Select(m => m.Horaire));
        }
        ...
        {
            return SingleResult.Create(db.Plannings.Where(m => m.id == key).Select(m => m.Jour));
        }
        ...
        {
            return SingleResult.Create(db.Plannings.Where(m => m.id == key).Select(m => m.Lieu));
        }

        // GET: odata/Plannings(5)/Mois
        [EnableQuery]
        public SingleResult<Mois> GetMois([FromODataUri] int key)
        {
            return SingleResult.Create(db.Plannings.Where(m => m.id == key).Select(m => m.Mois));
        }

        // GET: odata/Plannings(5)/NumeroJour
        [EnableQuery]
        public SingleResult<NumeroJour> GetNumeroJour([FromODataUri] int key)
        {
            return SingleResult.Create(db.Plannings.Where(m => m.id == key).Select(m => m.NumeroJour));
        }
        ...
        {
            return db.Plannings.Count(e => e.id == key) > 0;
        }
    }
}
